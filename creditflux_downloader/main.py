...
if __name__ == '__main__':
    ...
    stop = False
    while(stop != True):
        choose = input('Single CLO? (yes or no): ')
        if (choose == 'yes'):
            page = ExtractDataPage(chromedriver_path=os.path.join(global_folder, 'chromedrivers\windows\chromedriver.exe'))
            x = input('Single CLO Name:')
            print('downloading all the results of ', x)
            page.download(x)
            print('done')
        else:
            x = input('Filepath of the txt file with your CLO list:')
            y = input('Number of threads(no greater than 4): ')
            print('downloading all the results of ', x)
            download_multiple(x, num_threads=int(y))
            print('done')
        another = input('Would you like to download more? (yes or no): ')
        if (another == 'no'):
            stop = True
        else:
            stop = False